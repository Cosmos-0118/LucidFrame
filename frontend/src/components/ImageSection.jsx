...
function ImageSection({ backendUrl, warmup }) {
  ...
  useEffect(() => {
    if (!beforeUrl && !afterUrl && !loading) {
      setStatus(warmup ? "Warming backend…" : "Drop an image to start");
    }
  }, [afterUrl, beforeUrl, loading, warmup]);
  ...
  const startImageJob = useCallback(
    ...
    [analyzeImageFile, autoEnhance, backendUrl, brightness, contrast, denoiseStrength, exposure, faceRestore, hashFile, mode, presetIntensity, resetImagePreview, saturation, scale, sharpenStrength, textSafe, tonePreset],
  );
